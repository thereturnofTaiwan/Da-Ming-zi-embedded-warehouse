...
//START 任务
//设置任务优先级
#define START_TASK_PRIO (20)  //开始的任务优先级设置为最低
//设置任务堆栈大小
...
int main(void)
{
  ...
  OSTaskCreate(pulse_task,(void *)0,(OS_STK *)&PULSE_TASK_STK[PULSE_STK_SIZE-1],PULSE_TASK_PRIO );      //创建脉冲任务
  OSTaskCreate(serial_task,(void *)0,(OS_STK *)&Serial_TASK_STK[Serial_STK_SIZE-1],Serial_TASK_PRIO );  //创建串口通信任务
  OSStart();
  /* USER CODE END 2 */
...
void usart_task(void *pdata)
{
  while(1)
  {
      pulse_num = 10;
      Data[0] = pulse_num;
      HAL_UART_Transmit_DMA(&huart1, Data, 0xffff);
      HAL_UART_Receive_DMA(&huart1, Data, 0xffff);
